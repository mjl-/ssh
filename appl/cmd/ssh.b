	...
	for(;;) {
		...
		Sshlib->SSH_MSG_KEXDH_REPLY =>
			...
			dhhash := sha1bufs(list of {
				valpack(ref Val.Str (array of byte lident)),
				valpack(ref Val.Str (array of byte rident)),
				valpack(ref Val.Str (clkexinit)),
				valpack(ref Val.Str (srvkexinit)),
				valpack(srvksval),
				mpintpack(dhe),
				valpack(srvfval),
				mpintpack(key)});
			...
			keypack := valpack(ref Val.Mpint(key));
		...
		Sshlib->SSH_MSG_USERAUTH_SUCCESS =>
			...
			err = writepacket(c, Sshlib->SSH_MSG_CHANNEL_OPEN, vals);
			if(err != nil)
				fail(err);
		...
		Sshlib->SSH_MSG_CHANNEL_OPEN_CONFIRMATION =>
			...
			# uint32    maximum packet size
			# ....      channel type specific data follows
			(a, err) = parsepacket(d[1:], list of {Tint, Tint, Tint, Tint});
			if(err != nil)
				fail(err);
			...
			err = writepacket(c, Sshlib->SSH_MSG_CHANNEL_REQUEST, vals);
			if(err != nil)
				fail(err);
		...
		Sshlib->SSH_MSG_CHANNEL_SUCCESS =>
			cmd("### channel success");
			(nil, err) = parsepacket(d[1:], list of {Tint});
			if(err != nil)
				fail(err);
		...
		Sshlib->SSH_MSG_CHANNEL_DATA =>
			...
			# uint32    recipient channel
			# string    data
			(a, err) = parsepacket(d[1:], list of {Tint, Tstr});
			if(err != nil)
				fail(err);
			say("channel data:\n"+string getstr(a[1]));
		...
		Sshlib->SSH_MSG_CHANNEL_EXTENDED_DATA =>
			...
			# string    data
			(a, err) = parsepacket(d[1:], list of {Tint, Tint, Tstr});
			if(err != nil)
				fail(err);
			datatype := getint(a[1]);
			case datatype {
		...
		Sshlib->SSH_MSG_CHANNEL_OPEN_FAILURE =>
			...
			# string    language tag [RFC3066]
			(a, err) = parsepacket(d[1:], list of {Tint, Tint, Tstr, Tstr});
			if(err != nil)
				fail(err);
...
valpack(v: ref Val): array of byte
{
	n := v.size();
	d := array[n] of byte;
	v.pack(d);
...
{
	return valpack(ref Val.Mpint (v));
}
...
#knownkex := array[] of {"diffie-hellman-group-exchange-sha1"};
