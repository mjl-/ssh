...
interaction(attrs: list of ref Attr, io: ref IO): string
{
	...
	for(; keys != nil; keys = tl keys){
		...
		if(sk != nil){
			r := sys->aprint("%s %s", sk.pk.ek.iptostr(16), sk.pk.n.iptostr(16));
...
{
	# xxx the keys openssh's ssh-keygen generated have use different values for kp/kq/c2.
	# when i move to using our own generated key, i'll switch back to the old code.
if(1) {
	{
		ek := ipint(k.attrs, "ek");
		n := ipint(k.attrs, "n");
		dk := ipint(k.secrets, "!dk");
		p := ipint(k.secrets, "!p");
		q := ipint(k.secrets, "!q");
		return (RSAsk.fill(n, ek, dk, p, q), nil);
	}exception e{
	Badkey =>
